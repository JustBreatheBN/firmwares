...
//unsigned long currentMillisasd = 0;
...
const int MQTT_BUFFER_SIZE = 1200;
char mqttBuffer[MQTT_BUFFER_SIZE];
StaticJsonDocument<1200> asdoc;
StaticJsonDocument<1200> sensorStatus;
// Function prototypes
...
void loop() {
  ...
  if (buttonState == LOW && lastButtonState == HIGH) {
    ...
    if (millis() - buttonPressTime >= longPressTime) {
      Serial.print("Button long pressed for 10 seconds");
      handleResetConfig();
    }
  }
  ...
  if (currentMillisasd - previousMillisasd >= asdFreq * 1000) { //60000
    ...
    asdoc.clear();
    if (deviceId != "") {
      asdoc["device_id"] = deviceId;
    }
    else {
      asdoc["device_id"] = "Noid";
    }
    asdoc["pm1"] = pms25_data[0];
    asdoc["pm2_5"] = pms25_data[1];
    asdoc["pm10"] = pms25_data[2];

    // Applying rounding to 2 decimal places for the required fields
    asdoc["temperature1"] = t;  // Limit temperature1 to 2 decimal places
    asdoc["temperature2"] = t1;  // Limit temperature2 to 2 decimal places
    asdoc["humidity"] = h;  // Limit humidity to 2 decimal places
    asdoc["lux"] = lux;  // Limit lux to 2 decimal places
    asdoc["db"] = sound_level;  // Decibel level remains the same

    asdoc["tvoc"] = sgp30_TVOC;  // TVOC remains the same
    if (co2Found) {
      asdoc["co2"] = fetchCo2Ppm();  // CO2 remains the same
    }
    else {
      asdoc["co2"] = 0.0;  // CO2 remains the same
    }

    asdoc["so2"] = so2;  // so2 remains the same
    asdoc["o2"] = o2;  // Limit O2 to 2 decimal places
    asdoc["o3"] = o3;  // O3 remains the same
    asdoc["co"] = co;  // CO remains the same
    asdoc["no2"] = no2;  // NO2 remains the same
    asdoc["nh3"] = nh3;  // NH3 remains the same

    // Formatting the date and time
    if (now != "") {
      String dateTime = String(now.year()) + "-" + String(now.month()) + "-" + String(now.day()) + " " + String(now.hour()) + ":" + String(now.minute()) + ":" + String(now.second());
      asdoc["dateTime"] = dateTime;
    }
    
    serializeJson(asdoc, Serial2);
//    publishToMQTT(PUB_asd, asdoc.as<JsonObject>());
...
void saveConfiguration(const JsonObject& config) {
  ...
  asdFreq = preferences.getInt("asdFreq", 30);
