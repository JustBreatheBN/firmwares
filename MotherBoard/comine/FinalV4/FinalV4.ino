...
String outSensorID;     // Default to an empty string
float version;
int update;
...
int npkPin;  // Assuming pin 9 is fixed for this logic
int npkDelay = 0;
int mdFreq = 1;
float amp = 0, volt = 0;
...
//  Serial.println("Wifi COnnected");
  ...
  // Initialize GPIO pins
    ...
    // Read the initial state of the pins
...
//    Serial.println(message);
    ...
    StaticJsonDocument<600> receivedData;
    ...
    if (!err) {
        ...
        if (currentMillisasd - previousMillisasd >= asdFreq * 1000) { // 60000
            ...
            if (deviceId != "") {
                serializeJson(receivedData, mqttBuffer, MQTT_BUFFER_SIZE);
                Serial.print("Publishing relay feedback data: ");
                Serial.println(mqttBuffer);

                if (client.connected()) { // Ensure MQTT client is connected
                    bool success = client.publish(PUB_asd.c_str(), mqttBuffer);
                    if (success) {
                        Serial.println("Relay feedback data published successfully");
                    } else {
                        Serial.print("Failed to publish relay feedback data, MQTT state: ");
  ...
  if (Serial.available() > 0) {
    ...
    if (message == "status") {
     ...
     else if (message == "powStatus") {
      // Create a JSON document
     int currentPowSupStatus = digitalRead(powSup);
    int currentEarSupStatus = digitalRead(earSup);

    // Create a JSON document
    StaticJsonDocument<200> powerStatus;

    // Add power supply status
    powerStatus["powerSupply"] = (currentPowSupStatus == LOW) ? "OK" : "FAIL";

    // Add earthing status
    powerStatus["earthing"] = (currentEarSupStatus == LOW) ? "OK" : "FAIL";

    // Serialize the JSON object and send it over the serial port
    serializeJsonPretty(powerStatus, Serial);
//    Serial.println();
    }
    else {
      //      Serial.println(message);
...
//  Serial.println();
  ...
  if (strcmp(topic, SUB_Config.c_str()) == 0) {
    ...
    irrDelay = preferences.getInt("irrDelay", 5);
...
void loadConfiguration() {
  ...
  if (!preferences.begin("config", true)) {  // Open in read-only mode
    ...
    outSensorID = "";     // Default to an empty string
    version = 1.0;
    update = 0;
  ...
  asdFreq = preferences.getInt("asdFreq", 1);
  mdFreq = preferences.getInt("mdFreq", 1);
  irrDelay = preferences.getInt("irrDelay", 30);
  irrPin = preferences.getInt("irrPin", 1);
  npkDelay = preferences.getInt("npkDelay", 30);
