...
void handleSaveDeviceConfig() {
  if (server.method() == HTTP_POST) {
    ...
    SUB_updates = "Devices/C2M/" + deviceId + "/updates";
    PUB_updates = "Devices/M2C/" + deviceId + "/updates";
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (input.length() > 0) {
      ...
      if (docT.containsKey("mbv")) {
        // Check for the URL in the JSON.
        if (docT.containsKey("mbu")) {
          const char* url = docT["mbu"];
  ...
  if (WiFi.status() == WL_CONNECTED) {
    if (THINGNAME != "" && deviceId != "") {
      ...
      if (client.connected()) {
        ...
        client.subscribe(SUB_updates.c_str());
...
void messageHandler(char* topic, byte* payload, unsigned int length) {
  ...
  else if (strcmp(topic, SUB_updates.c_str()) == 0) {
    Serial.println("Processing UPdates data...");
    //    doc["mbv"];
    //    doc["mbu"];
    //    doc["rbv"];
    //    doc["rbu"];
    // Check if the command contains "update": 1.
    if (doc.containsKey("mbv")) {
      // Check for the URL in the JSON.
      if (doc.containsKey("mbu")) {
        const char* url = doc["mbu"];
        Serial.print("Starting OTA update with URL: ");
        Serial.println(url);
