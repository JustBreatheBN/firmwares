...
void initializePwmPins() {
  for (int i = 0; i < 2; i++) {
    ...
    analogWrite(pwmPins[i], 255); // Initialize PWM pins to 0
    regulateStatusDoc[String(i + 1)] = 255; // Initialize PWM status in JSON
  }
}
...
//            int ddutyCycle = map(pwmValue, 0, 100, 255, 0);
//            analogWrite(pwmNum, ddutyCycle);
  ...
  for (int i = 0; i < 2; i++) {
    if (pwmControl[i].isRamping) {
      if (currentMillis - pwmControl[i].lastUpdate >= pwmStepDelay) {
        ...
        int dutyCycle = map(pwmControl[i].currentPWM, 0, 100, 255, 0);
        analogWrite(pwmPins[i], dutyCycle);
